...
BOOST_AUTO_TEST_CASE(block_type_classification)
{
    ...
    // SUPER_AXIS blocks: 9, 12, 18 (also AXIS)
    ...
    BOOST_CHECK_EQUAL(GetTriadicBlockType(12), TriadicBlockType::SUPER_AXIS);
    BOOST_CHECK(IsAxisBlock(12));
    BOOST_CHECK(IsSuperAxisBlock(12));
    BOOST_CHECK_EQUAL(GetTriadicBlockType(18), TriadicBlockType::SUPER_AXIS);
    ...
    BOOST_CHECK(!CanReorganizeTo(12));
...
// ============================================================
// 4. BlockMerkleRoot for regtest genesis
// ============================================================
...
// ============================================================
// 5. Regtest genesis PoW is valid (nonce=6175, nBits=0x207fffff)
// ============================================================
BOOST_AUTO_TEST_CASE(regtest_genesis_pow)
{
    ...
    // nonce=1, nBits=0x207fffff (regtest difficulty)
    BOOST_CHECK_EQUAL(genesis.nNonce, 1U);
    BOOST_CHECK_EQUAL(genesis.nTime, 1775858400U);
}

// ============================================================
// 6. Mainnet genesis PoW is valid (nonce=6175, nBits=0x1d00ffff)
// ============================================================
BOOST_AUTO_TEST_CASE(mainnet_genesis_pow)
{
    auto params = CreateChainParams(*m_node.args, ChainType::MAIN);
    const CBlock& genesis = params->GenesisBlock();
    const auto& consensus = params->GetConsensus();

    bool powOk = CheckProofOfWork(genesis.GetHash(), genesis.nBits, consensus);
    fprintf(stderr, "DEBUG_MAIN powOk=%d hash=%s nBits=0x%x\n", powOk, genesis.GetHash().ToString().c_str(), genesis.nBits);
    BOOST_CHECK_MESSAGE(powOk, "Mainnet genesis PoW should be valid");
    BOOST_CHECK_EQUAL(genesis.nNonce, 6175U);
    BOOST_CHECK_EQUAL(genesis.nTime, 1775858400U);
}

// ============================================================
// 7. Signet genesis PoW is valid (nonce=2)
// ============================================================
BOOST_AUTO_TEST_CASE(signet_genesis_pow)
{
    auto params = CreateChainParams(*m_node.args, ChainType::SIGNET);
    const CBlock& genesis = params->GenesisBlock();
    const auto& consensus = params->GetConsensus();

    bool powOk = CheckProofOfWork(genesis.GetHash(), genesis.nBits, consensus);
    BOOST_CHECK_MESSAGE(powOk, "Signet genesis PoW should be valid");
    BOOST_CHECK_EQUAL(genesis.nNonce, 2U);
}

// ============================================================
// 8. All genesis hashes equal their consensus values (chainparams self-consistency)
// ============================================================
BOOST_AUTO_TEST_CASE(all_genesis_hashes)
{
    auto check_genesis = [this](ChainType ct, const char* name) {
        auto params = CreateChainParams(*m_node.args, ct);
        const CBlock& genesis = params->GenesisBlock();
        const auto& consensus = params->GetConsensus();
        BOOST_CHECK_MESSAGE(
            genesis.GetHash() == consensus.hashGenesisBlock,
            name << " genesis.GetHash() should equal consensus.hashGenesisBlock");
        BOOST_CHECK_MESSAGE(
            CheckProofOfWork(genesis.GetHash(), genesis.nBits, consensus),
            name << " genesis PoW must be valid");
    };

    check_genesis(ChainType::REGTEST, "regtest");
    check_genesis(ChainType::MAIN, "mainnet");
    check_genesis(ChainType::SIGNET, "signet");
}

// ============================================================
// 9. Hash(a || b) != Hash(b || a) — order matters for AXIS merkle
// ============================================================
...
// ============================================================
// 10. Super AXIS block (height 9) is also an AXIS block
// ============================================================
BOOST_AUTO_TEST_CASE(super_axis_is_also_axis)
{
    ...
    BOOST_CHECK(IsSuperAxisBlock(18));
    BOOST_CHECK(IsAxisBlock(18));
